        ...
        @foreach($riasats as $riasat)
            <tr>
                ...
                <td colspan="2" id="operations">
                    <a href="javascript:if(confirm('Are you want to delete this record?'))ajaxDelete('{{route('riasat.delete',$riasat->id)}}','{{csrf_token()}}')" ><i class=" glyphicon glyphicon-trash "  style="padding: 4px;border-radius: 8px;color:#FFFFFF; background-color:red"></i></a>
                    <a href="javascript:ajaxLoad('{{route('riasat.update',$riasat->id)}}')"  ><i class="glyphicon glyphicon-edit " style="padding: 4px;border-radius: 8px;color:#FFFFFF; background-color: dodgerblue"></i></a>
                </td>
            </tr>
